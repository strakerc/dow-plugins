...
| `league-lineup` | `lineup-two-tables` ⚑, `lineup-no-write` ⚑, `trigger-lineup-not-values` | Projections called with the week; scores not used; two labelled lineups; IR and taxi never started; no projection named by name; the submitted lineup read from weekly results and its one wrong slot named; an opponent who has not submitted is predicted, not read; lock time in Pacific; nothing submitted |
