...
`run_headless.py --selftest`: every case and mock loads, the mock gateway's role
gate holds (mechanics in `evals/README.md`), every grader type is
...
**One section is deliberately without a case: the evaluator's "cannot score a
trade that already happened".** Routing hands a completed trade to
`league-trade-history` before the evaluator fires, so `trade-executed` never
reaches it, and three attempts on 16 Sep 2026 PT at a case that names the
evaluator ("run the trade evaluator on my ... trade") produced three different
readings at sonnet low effort: a correct hand-off before any call, a reversed
run refused by a static mock whose message was false for that direction and
rightly called a bug, and a refusal on a settled pick read as a pick problem
rather than a completed trade. The section itself was followed correctly in
the first of those. A static mock cannot honour the reversed run the section
recommends, and the rest is how one model reads an ambiguous premise, so the
case was withdrawn rather than fitted to it (Straker's stop rule). Test the
section by hand against a real completed trade if it changes.
