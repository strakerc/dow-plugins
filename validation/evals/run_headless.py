...
`--plugin-dir` pointing at the plugin(s) under plugins/, `--strict-mcp-config`
so the only tool server is validation/evals/mock_mcp_server.py, WebFetch and
WebSearch disallowed, and the case prompt as the whole conversation. Cases
...
DISALLOWED = ["WebFetch", "WebSearch", "Write", "Edit", "NotebookEdit", "Agent"]
...
def run_case_once(case, args, claude, results_dir, run_no):
    ...
    for pl in case["plugins"]:
        cmd += ["--plugin-dir", str(PLUGINS / pl)]
    allowed = ["Skill", "Read", "Glob", "Grep", "Bash(python3 *)", "Bash(python *)"]
    if case["mocks"]:
    ...
    meta = {"type": "dow-meta", "case": case["name"], "model": model,
    ...
    try:
        p = subprocess.run(cmd, input=case["prompt"], capture_output=True, text=True,
                           encoding="utf-8", cwd=tmp, timeout=args.timeout)
...
def unreachable(t):
    ...
    a run that produced nothing, or the CLI's short "not logged in" reply.
...
def record_now(case, passed, model, results_dir, skills_fp_value=None, tools=None):
    ...
    with _LEDGER_LOCK:
        led = read_ledger()
        record(led, case, passed, model, results_dir, skills_fp_value, tools)
        write_ledger(led)
...
def ledger_status(case, led, model):
    """'current' (a pass recorded against exactly these files and this model),
    'failed', 'stale' (files changed since), or 'new'."""
    e = led.get(case["name"])
    if not e:
        return "new"
...
def record(led, case, passed, model, results_dir, skills_fp_value=None, tools=None):
    led[case["name"]] = {"passed": bool(passed), "model": model,
...
def regrade(args):
    ...
    for f in sorted(src.glob("*.run*.jsonl")):
        ...
        if args.case and not (fnmatch.fnmatch(c["dir"].name, args.case) or fnmatch.fnmatch(name, args.case)):
            continue
        model = meta.get("model") or led.get(name, {}).get("model") or args.model
        if not args.all and ledger_status(c, led, model) == "current":
        ...
        sfp = meta.get("skills_fp") or led.get(name, {}).get("skills_fp")
        if sfp:
            record_now(c, ok, model, src, sfp, tools=bare_tools(t["tools"]))
        else:
...
def print_ledger(args):
    ...
    counts = {}
    for c in cases:
        st = ledger_status(c, led, args.model)
        counts[st] = counts.get(st, 0) + 1
        e = led.get(c["name"], {})
        print(f"  {st:<8} {c['name']:<44} {e.get('when', '')[:19]} {e.get('model', '')}")
...
def main_run(args):
    ...
    led = read_ledger()
    if not args.all:
        status = {c["name"]: ledger_status(c, led, c["model"] or args.model) for c in cases}
        current = [c for c in cases if status[c["name"]] == "current"]
        cases = [c for c in cases if status[c["name"]] != "current"]
        for c in current:
            print(f"  current  {c['name']}  (passed {led[c['name']]['when'][:19]}, files unchanged; not rerun)")
        if not cases:
    ...
    def run_case(c):
        ...
        if runs and not any(r.get("unreachable") for r in runs):
            record_now(c, passed, c["model"] or args.model, results_dir,
                       tools=bare_tools(t["tools"]))   # per case: a killed run keeps its passes
        return entry
    ...
    if dead:
        print("MODEL UNREACHABLE: the suite did not run. Nothing above is a pass.\n"
              "  Log the CLI in from a real terminal:  claude auth login")
        return 2
    return 1 if any_fail else 0
...
def selftest():
    ...
    check("ledger: other model is stale", ledger_status(probe, led, "haiku") == "stale")
    led[probe["name"]]["evals_fp"] = "changed"
    check("ledger: changed case files are stale", ledger_status(probe, led, "sonnet") == "stale")
    record(led, probe, True, "sonnet", Path("x"))
    led[probe["name"]]["skills_fp"] = "changed"
